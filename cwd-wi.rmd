...
wi.small.subset <- c("Dane", "Columbia", "Dodge", "Jefferson")

cwd.dat.county.sep.sub <- cwd.dat.county.sep[cwd.dat.county.sep$county %in% wi.cwd.area,]
...
studyarea <- studyarea[-c(which(!c(studyarea$FID) %in% c(cwd.dat.pos$fid))),]
studyarea <- studyarea[-c(which(duplicated(studyarea$FID))),] # get rid of duplicates
cwd.dat.pos <- cwd.dat.pos[-c(which(duplicated(cwd.dat.pos$fid))),]
cwd.dat.analyzed <- cwd.dat.analyzed[-c(which(duplicated(cwd.dat.analyzed$fid))),]
...
studyarea<-arrange(studyarea, studyarea$FID)
cwd.dat.pos<-arrange(cwd.dat.pos, cwd.dat.pos$fid)
cwd.dat.analyzed<-arrange(cwd.dat.analyzed, cwd.dat.analyzed$fid)

cwd.mat<-matrix(0, nrow=length(unique(cwd.dat.pos$fid)), ncol=n.yrs)
cwd.mat.anlyzd <- cwd.mat
sites<-unique(cwd.dat$fid)
...
# k<-1
# for(i in 1:nrow(cwd.dat.pos)){
#   for(j in 2:length(n.yrs-1)){
#         cwd.mat[i,k] = cwd.dat.pos[i,j]
#         cwd.mat.anlyzd[i,k] = cwd.dat.analyzed[i,j] 
#         k = k + 1
#         }
# }
# k<-1
  ...
  for (i in 1 : n.twnshps) {
    ...
    gamma0 ~dnorm(0.0, sd=100)
       ...
       # why not try the average for particular year across all sampling units -- later we can try to make it the average across across a cluster of sampling units
        ...
        log(mu[i,j]) <-  log(s[i,j]*mu.hat) + beta0 
        logit(p[i,j]) <- log(s[i,j]*mu.hat) + gamma0
