...
# cwd.dat.s <- cwd.dat%>%subset(year >= 2013)
# cwd.dat.s$wsi <- 0
# # 2014 - 2022
# # years <- c(1:8)
...
#         if(cwd.dat.s[i,]$year == year.samp[j]){
#           if(cwd.dat.s[i,]$county == "Dane"){
#             cwd.dat.s[i,]$wsi <- wsi[j,]$Dane
#           }
#           if(cwd.dat.s[i,]$county == "Dodge"){
#             cwd.dat.s[i,]$wsi <- wsi[j,]$Dodge
#           }
#           if(cwd.dat.s[i,]$county == "Jefferson"){
#             cwd.dat.s[i,]$wsi <- wsi[j,]$Jefferson
#           }
#           if(cwd.dat.s[i,]$county == "Columbia"){
#             cwd.dat.s[i,]$wsi <- wsi[j,]$Columbia
#           }
#             (cwd.dat.s[i,]$county)
#         }
#     else{
...
wiTwnshipShp$cwd.expected <- 0
# associate with wiTwnShpShp
for(i in 1:nrow(wiTwnshpShp)){
  for(j in 1:nrow(cwd.dat)){
    if(wiTwnshpShp[i,]$TWP)
  }
}
...
modelcode <- nimbleCode({
    ...
    for (i in 1 : N) {
      for(j in 1 : n.cases){
        y[i] ~ dZIP(mu[i], p[i])
        log(mu[i]) <- beta0 + beta1*aff[i] + phi[i] + theta[i]
...
constants <- list(N = nrow(wiTwnshpShp), L = length(W$adj), adj=W$adj, weights=W$weights, num=W$num,
                  E = data.spPoly$cwd.pos, aff = data.spPoly$pcaff)
